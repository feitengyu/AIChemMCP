...
from tools.Hydrogel_server_tools import ActionServerTools
...
def Hydrogel_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [Hydrogel_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)


def Hydrogel_server_advertise_capabilities():
    ...
    print(f"--- [Hydrogel_Server] 水凝胶工作站类型 is ready. ---", file=sys.stderr, flush=True)


if __name__ == "__main__":
    # 启动流程：先广播能力，再进入主循环
    Hydrogel_server_advertise_capabilities()
    Hydrogel_server_main_loop()
