...
from tools.LiquidNitrogen_server_tools import ActionServerTools
...
# --- MCP协议通信主逻辑（自动适配当前设备）---
def LiquidNitrogen_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [LiquidNitrogen_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)


def LiquidNitrogen_server_advertise_capabilities():
    ...
    "num_Liquid_N": {
        ...
        "description": "num_Liquid_N"
                ...
                "required": ["data", "num_Lc", "temp", "num_Liquid_N"]
            }
        }
                    ]
                }
    ...
    print(f"--- [LiquidNitrogen_Server] 液氮工作站 is ready. ---", file=sys.stderr, flush=True)


if __name__ == "__main__":
    # 启动流程：先广播能力，再进入主循环
    LiquidNitrogen_server_advertise_capabilities()
    LiquidNitrogen_server_main_loop()
