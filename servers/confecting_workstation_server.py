...
from tools.confecting_workstation_server_tools import ActionServerTools
...
def confecting_workstation_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [confecting_workstation_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def confecting_workstation_server_advertise_capabilities():
    ...
    print(f"--- [confecting_workstation_Server] 电化学-配液仪 is ready. ---", file=sys.stderr, flush=True)
