...
from tools.elec_chem_storage_workstation_server_tools import ActionServerTools
...
def elec_chem_storage_workstation_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [elec_chem_storage_workstation_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def elec_chem_storage_workstation_server_advertise_capabilities():
    ...
    print(f"--- [elec_chem_storage_workstation_Server] 置物工作站 is ready. ---", file=sys.stderr, flush=True)
