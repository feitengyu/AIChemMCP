...
from hts_server_tools import ActionServerTools
...
def hts_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [hts_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)


def hts_server_advertise_capabilities():
    ...
    print(f"--- [hts_Server] 合成工作站 is ready. ---", file=sys.stderr, flush=True)


if __name__ == "__main__":
    # 启动流程：先广播能力，再进入主循环
    hts_server_advertise_capabilities()
    hts_server_main_loop()
