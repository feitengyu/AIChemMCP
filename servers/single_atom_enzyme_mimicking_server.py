...
from tools.single_atom_enzyme_mimicking_server_tools import ActionServerTools
...
def single_atom_enzyme_mimicking_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [single_atom_enzyme_mimicking_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def single_atom_enzyme_mimicking_server_advertise_capabilities():
    ...
    print(f"--- [single_atom_enzyme_mimicking_Server] 单原子仿酶催化平台 is ready. ---", file=sys.stderr, flush=True)
