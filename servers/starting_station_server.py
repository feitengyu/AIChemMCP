...
from tools.starting_station_server_tools import ActionServerTools
...
def starting_station_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [starting_station_Server] Critical Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def starting_station_server_advertise_capabilities():
    ...
    print(f"--- [starting_station_Server] 物料站 is ready. ---", file=sys.stderr, flush=True)
