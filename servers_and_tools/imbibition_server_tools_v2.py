...
class UnifiedWorkstationTools:
    ...
    def tool_imbibition(self, task_description: str, **params):
        """
        吸液工作站 - 自动液体处理工作站是一种多功能的液体处理设备。它可以进行精确的移液、分液、混合等操作，并且能够根据不同的实验需求进行灵活编程。其配备的高精度液体处理模块和智能控制系统，确保了液体处理的准确性和重复性。在药物研发、临床检验等领域，可用于高通量的样品处理，如药物筛选实验中的样品稀释、混合等操作，提高实验效率和质量。
        
        可用操作:
        - 将离心机里的瓶子转移至吸液工位后启动吸液 (to_Aspirate): None
        - 测试 (test): None
