...
# --- 定义统一的工作站工具函数 ---
def tool_imbibition(task_description: str, **params):
    """
    执行吸液工作站的任务
    ...
    """
    return tool_manager.tool_imbibition(task_description, **params)



AVAILABLE_TOOLS = {
    "吸液工作站": tool_imbibition
}


# --- MCP协议通信主逻辑 ---
def imbibition_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [imbibition_Server] Error: {str(e)} ---", file=sys.stderr, flush=True)


def imbibition_server_advertise_capabilities():
    """广播当前设备的MCP能力"""
    adv_message = {
        "jsonrpc": "2.0",
    ...
    print(f"--- [imbibition_Server] 吸液工作站 已就绪 ---", file=sys.stderr, flush=True)


if __name__ == "__main__":
    # 启动流程：先广播能力，再进入主循环
    imbibition_server_advertise_capabilities()
    imbibition_server_main_loop()
