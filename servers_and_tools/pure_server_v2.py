...
# --- 定义统一的工作站工具函数 ---
def tool_pure(task_description: str, **params):
    """
    执行纯化工作站的任务
    ...
    """
    return tool_manager.tool_pure(task_description, **params)
...
def pure_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [pure_Server] Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def pure_server_advertise_capabilities():
    ...
    adv_message = {
        ...
        {
            "name": "纯化工作站",
            "description": "离心纯化工作站主要用于对各类混合物进行离心、清洗或纯化处理。它采用机械臂、震荡机及离心机高度耦合的方式，实现从进样、清洗、离心或纯化的完全无人操作。在化学、生物等实验中，目标产物往往需要进行固相和液相分离，经过清洗得到无杂质的样品，离心纯化工作站能够高效的进行分离获取需要的目标产物。",
            "parameters": {
                "type": "object",
                "properties": {
                    "task_description": {
    ...
    print(f"--- [pure_Server] 纯化工作站 已就绪 ---", file=sys.stderr, flush=True)
