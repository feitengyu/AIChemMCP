...
# --- 定义统一的工作站工具函数 ---
def tool_ustc_mul_solid(task_description: str, **params):
    """
    执行多通道固体进样器的任务
    ...
    """
    return tool_manager.tool_ustc_mul_solid(task_description, **params)



AVAILABLE_TOOLS = {
    "多通道固体进样器": tool_ustc_mul_solid
}
...
def ustc_mul_solid_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [ustc_mul_solid_Server] Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def ustc_mul_solid_server_advertise_capabilities():
    ...
    print(f"--- [ustc_mul_solid_Server] 多通道固体进样器 已就绪 ---", file=sys.stderr, flush=True)
