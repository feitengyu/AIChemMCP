...
# --- 定义统一的工作站工具函数 ---
def tool_ustc_photocatalysis(task_description: str, **params):
    """
    执行光催化的任务
    ...
    """
    return tool_manager.tool_ustc_photocatalysis(task_description, **params)



AVAILABLE_TOOLS = {
    "光催化": tool_ustc_photocatalysis
}
...
def ustc_photocatalysis_server_main_loop():
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps(response, ensure_ascii=False), flush=True)
            print(f"--- [ustc_photocatalysis_Server] Error: {str(e)} ---", file=sys.stderr, flush=True)
...
def ustc_photocatalysis_server_advertise_capabilities():
    ...
    print(f"--- [ustc_photocatalysis_Server] 光催化 已就绪 ---", file=sys.stderr, flush=True)
